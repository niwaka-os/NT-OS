...
typedef struct vim_data{
    ...
    int vim_file_num;     
    char file_exist_flg;    //1:もともと存在していたファイル  0:もともとなかったファイル
}VIM;
        ...
        while(1){
            ...
            if(key_bf->data_num>=1){
                ...
                if(data == 0x1c){
                    ...
                    cons->command_num -= 1;//Enterキーは関係ない。
                    //コマンドチェックを行う。
...
int check_command(CONSOLE *cons){
    ...
    //cons->command[len-1] = ' ';       //コマンド

    char LS_COMMAND[]="LS";           //LSコマンド
    char VIM_COMMAND[]="VIM";         //VIMコマンド
    char MAKE_COMMAND[] = "MAKE";     //MAKEファイルコマンド
    //int cmp_name_firstarg(char *s1, char *s2, int num);戻り値-1:失敗  1:成功
    //コマンドチェックを行う。
    if(cmp_name_firstarg(&LS_COMMAND[0], &cons->command[0], 2)==1){   

        return 0;   //コマンド実行を行う。
    } else if(cmp_name_firstarg(&VIM_COMMAND[0], &cons->command[0], 3)==1){
        //VIMの後に、ファイルを入力しているかチェックする。
        if(cons->command[3]==0x20){
            //ここでファイル名が存在するかチェックする。
            if(check_file_exist(&cons->command[4])==1){
                return 1;
            }
            return -1;//存在しなかった。
        }
                ...
                for(;;){
                    ...
                    if(data <= 127 && data>=0){
                        if(data==0x1f){//Sキーを押したら終了する。
                            ...
                            store_file(&vim->vim_file, &vim->filename);
                            console_reset(cons);
                            return;
                        }
...
void ls(FILE_ENTRY* entry, CONSOLE*cons){
	...
	for(i=0; i < entry_num; i++){
		...
		if(entry_now->filename[0] != 0){		
            ...
            console_setbuf(cons, ' ', cons->now_col, cons->now_row);
		}
	}
