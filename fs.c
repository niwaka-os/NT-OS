...
//ファイル保存
//保存したファイルの場所
//今後の修正予定→ファイルの保存は、新しいファイルの保存なのか、ファイルの更新なのかで区別して保存する機能を実装予定
...
//ファイル名に対応するルートディレクトリ領域の要素を取得する。
FILE_ENTRY* get_dirregion(char *filename){
    FILE_ENTRY *file_entry_now;
...
//フロッピーディスクから読み込んだデータをbufに転送する。
void send_floppy_to_buf(char *dest){
    int i;

    for(i=0;i<512;i++){
        *(dest+i) = *((char*)DMA_BUFF+i);
    }
...
//次のクラスタ番号を取得する。
short get_next_clust_number(short clust_number){
    return fat[clust_number];
}

//filename:開きたいファイル名  buf:ファイルの内容を転送する開始番地
void open_file(char *filename, char *buf){
    //ファイル名に対応するルートディレクトリ領域の要素を取得する。
    FILE_ENTRY *file_entry_now;
    file_entry_now = get_dirregion(filename);
    //filenameの最初のクラスタ番号をゲットする。
    short clust_startnumber;
    clust_startnumber = file_entry_now->first_logical_cluster;

    int clust_numbers;//ファイルに対応するクラスタの数
    int lba;
    int file_size;
    file_size = search_EOF(buf)+1;//fileのサイズを求める。
    clust_numbers = cal_clustnumber(file_size);

    int i;
    int clust_number;
    clust_number =clust_startnumber;

    //フロッピーディスクからデータを読み込んで、データを転送する。
    for(i=0; i < clust_numbers; i++){
        lba = cal_lbanumber_to_clust_number(clust_number);
        read_lba(lba);//読み込んだデータを*bufに転送する。
        //フロッピーディスクから、バッファに転送する。
        send_floppy_to_buf(buf+i*512);
        //次のクラスタ番号を取得する。
        clust_number = fat[clust_number];
        if(clust_number==0x0FFF){
            break;//終了する。
...
int check_file_exist(char *file_name){
    ...
    for(i=0; i < 224; i++){
        file_entry_now = (fat12_entry+i);
        //11文字比較を行う。
        if(cmp_name_firstarg(file_entry_now->filename, file_name, 11)==1){
            return 1;//あった
        }
    }
    return -1;//なかった
...
void make_file(char *filename){
    FILE_ENTRY *file_entry_now;//空きディレクトリ領域を探す。
    //ファイル名を設定する。
    //空きクラスタ番号を探す。
    int i;
    ...
    for(i=0; i < 224; i++){
        file_entry_now = (fat12_entry+i);
        if(file_entry_now->filename[0]==0){
            break;//あった
        }
    }   
    for(i=0; i < 11; i++){
